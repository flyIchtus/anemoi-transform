...
def define_grid_from_template(file: str) -> np.ndarray:
    """Define grid from npz file template"""

    grid_data = np.load(file)
    lat, lon = grid_data['latitude'], grid_data['longitude']
    lon = np.where(lon > 180, lon - 360, lon)
    return np.array([*zip(lat, lon)]) 
