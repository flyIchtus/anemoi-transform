...
class SuperOb(Filter):
    ...
    def forward(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Define output grid (spatial + temporal)
        if self.grid[0] == "h":
            output_grid = define_healpix_grid(int(self.grid[1:]))
        elif self.grid.endswith("npz"):
            output_grid = define_grid_from_template(self.grid)
        else:
        ...
        averaged_df = df.groupby(groupby_cols, observed=True, sort=False)[columns_to_average].agg(self.agg_method)
